...
window.validateForm = function(dom) {
	...
	var sourceErrorMsg = obj.attr("errorMsg");
	...
	if (isValidate) {
		...
		if (value == "") {
			//值为空时的提示信息,
			if(sourceErrorMsg == undefined){
				errorMsg = defaultRules.empty.message;
			}else{
				errorMsg = sourceErrorMsg;
			}
		} else {
			if (dataType != undefined) {
				var funcTypes = dataType.split(",");
				$.each(funcTypes, function(i, v){
					var paramsArray = [];
					var func=this;
					if (this.indexOf("[") > 0) {
						func = dataType.slice(0, this.indexOf("["));
						var params = dataType.slice(this.indexOf("[") + 1,
								this.lastIndexOf("]"));
						paramsArray = params.split("-");
					}

					var iskey = defaultRules.hasOwnProperty(func);

					if (iskey) {
						var regex_obj = defaultRules[func];
						...
						if (f) {
							...
							if(sourceErrorMsg == undefined) {
								if(errorMsg == "") {
									errorMsg = ErrorMsgFormat(regex_obj.message, paramsArray);
									return false;
								}
							} else {
								errorMsg = sourceErrorMsg;
...
var ErrorMsgFormat = function(value,param) {
	var reg = /{([^{}]+)}/gm;
	return value.replace(reg, function(match, name) {
		return param[name];
	});
};
